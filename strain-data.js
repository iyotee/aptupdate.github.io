    ...
    fetch(
      ...
      .then((data) => {
        ...
        if (
          ...
          showError.innerHTML = `<strong>Hey !</strong> The research query must be at least 2 characters 🤖`;
          showError.classList.remove("hidden");
          showError.classList.add("show");
          return;
        } else if (data.message === "0 strains found") {
          showError.innerHTML = `<strong>Hey !</strong> No strains found 🤖`;
          showError.classList.remove("hidden");
          showError.classList.add("show");
          return;
        }

        console.log(data.strains[0].name);
